...
function initializePartsVariables() {
    ...
    /** Mesh Deduction **/
    securityWindowMesh = Number($('.sec-win-mesh').text());
    securityDoorMesh = Number($('.sec-door-mesh').text());
    dgDoorMesh = Number($('.dg-door-mesh').text());
    dgWindowMesh = Number($('.dg-win-mesh').text());
    fibrDoorMesh = Number($('.fibr-door-mesh').text());
    fibrWindowMesh = Number($('.fibr-win-mesh').text());
    perfDoorMesh = Number($('.perf-door-mesh').text());
    perfWindowMesh = Number($('.perf-win-mesh').text());
}
...
function validate(type) {
    ...
    if (errorMsg) {
        //A title with a text under (using ubold theme js plugin)
        swal("", errorMsg);
        return false;
    }
